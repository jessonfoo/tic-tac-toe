var Config = {
  ...
  penColor: "rgb(0,0,0)",
  penHoverColor: "rgb(0,0,0)",
  penWinColor: "rgba(255,40,40,0.7)",
  penWidth: 8,
  penWinWidth: 12,
};
...
function AIMove(board, player){
  ...
  for(i = 0; i < outcomes.squares.length; i++){      
    ...
    testAiLogic = AiLogic(testBoard, -999, 999, player, false);

    if (testAiLogic > bestAiLogic){
      bestMove = outcomes.squares[i];
      bestAiLogic = testAiLogic;
    }
  }
...
function AiLogic(board, a, b, player, maximizingPlayer){
  var i, outcome = GetOutcomes(board),
      childBoard;
  ...
  if (maximizingPlayer){
    for(i = 0; i < outcome.squares.length; i++){
      ...
      a = Math.max(a, AiLogic(childBoard, a, b, player, false));
      if(b <= a){
        break; //b cut off
      }
    }
    ...
    for(i = 0; i < outcome.squares.length; i++){
      ...
      b = Math.min(b, AiLogic(childBoard, a, b, player, true));
      if (b <= a){
        break; //a cut off
      }
    }
...
resetBtn.addEventListener('click',onReset,true);
